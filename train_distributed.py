...
def main(unused_argv):
    ...
    # role name should be like worker-0, ps-0
    ...
    with tf.device(
        ...
        if FLAGS.sync_replicas:
            ...
            # Initial token and chief queue runners required
            # by the sync_replicas mode
        ...
        if is_chief:
            logger.info("Worker %d: Initializing session..." %
                        FLAGS.task_index)
        else:
        ...
        logger.info("Worker %d: Session initialization complete." %
                    FLAGS.task_index)
        ...
        while True:
            ...
            if local_step % 10 == 0:
                ...
                local_step += 1
                logger.info("Accuracy at local step %s: %s" %
                            (local_step, acc))
            else:
                # Training feed
